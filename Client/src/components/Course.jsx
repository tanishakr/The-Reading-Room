import React from "react";
import Cards from "./Cards";
import axios from "axios";
import { Link } from "react-router-dom";
import { useState } from "react";
import { useEffect } from "react";
import { getBook } from "../../../Server/controller/book.controller";

function Course() {
  const [book, setBook] = useState([]);
  useEffect(() => {
    const fetchbook = async () => {
      try {
        const res = await axios.get("http://localhost:3003/book");
        console.log(res.data);
        setBook(res.data)
      } catch (error) {
        console.log(error)
      }
    };
    fetchbook();
  }, []);

  return (
    <>
      <div className="max-w-screen-2xl container mx-auto md:px-25 px-5 py-2">
        <div className="mt-30 items-center justify-center text-center">
          <h1 className="text-2xl font-semibold md:text-4xl">
            <span className="text-green-500">Our Full Collection</span>
          </h1>
          <p className="mt-10">
            Welcome! You now have access to our entire library of premium and free titles. Use the search or browse below to find the perfect book for you, whether you're looking to learn something new or get lost in a great story. Your reading adventure starts here.
          </p>
          <Link to="/">
            <button className="bg-purple-600 text-white px-5 py-3 rounded-md hover:bg-orange-600 duration-300 mt-7">
              Back
            </button>
          </Link>
        </div>
        {/*
          - On small screens (`sm`), you'll have 2 cards.
          - On medium screens (`md`), you'll now have 3 cards per row.
          - On large screens (`lg`), you'll revert to 4 cards.
          This gives you granular control over the layout.
        */}
        <div className="mt-15 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
          {book.map((item) => (
            <Cards key={item.id} item={item} />
          ))}
        </div>
      </div>
    </>
  );
}

export default Course;
